...
import { z } from 'zod';
import { date, dateRangeQueryFields } from '../../core/dto';

const CreateTurfReviewSchema = z
  .object({
  ...
  .refine(
    (data) =>
      data.title || data.comment || (data.images && data.images.length > 0),
    {
...
const CreateTurfReviewDtoBase: ZodDto<typeof CreateTurfReviewSchema> =
  createZodDto(CreateTurfReviewSchema);
const UpdateTurfReviewDtoBase: ZodDto<typeof UpdateTurfReviewSchema> =
  createZodDto(UpdateTurfReviewSchema);
const TurfReviewFilterDtoBase: ZodDto<typeof TurfReviewFilterSchema> =
  createZodDto(TurfReviewFilterSchema);
const VoteReviewDtoBase: ZodDto<typeof VoteReviewSchema> =
  createZodDto(VoteReviewSchema);
const ReportReviewDtoBase: ZodDto<typeof ReportReviewSchema> =
  createZodDto(ReportReviewSchema);
const ModerateReviewDtoBase: ZodDto<typeof ModerateReviewSchema> =
  createZodDto(ModerateReviewSchema);
